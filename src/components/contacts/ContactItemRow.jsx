...
const ContactItemRow = ({ element, modalHandler }) => {
    ...
    return (
        <>
            <td className="px-3 py-2 hidden sm:table-cell">{element.name}</td>
            <td className="px-3 py-2">{element.email}</td>
            <td className="px-3 py-2 hidden md:table-cell">{element.subject}</td>
            <td className="px-3 py-2 hidden xl:table-cell">{element.query}</td>
            <td className="px-3 py-2">
                {DateTime.fromSQL(element.created_at).toLocaleString(DateTime.DATE)}
            </td>
            <td className="px-3 py-2 text-center w-1/12">{element.notified ? <i className="fa-regular fa-circle-check text-lime-600"></i> : <i className="fa-regular fa-circle-xmark text-orange-600"></i>}</td>
